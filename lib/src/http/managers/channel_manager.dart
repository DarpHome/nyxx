...
class ChannelManager extends ReadOnlyManager<Channel> {
  ...
  Future<void> updatePermissionOverwrite(Snowflake id, PermissionOverwriteBuilder builder, {String? auditLogReason}) async {
    final route = HttpRoute()
      ..channels(id: id.toString())
      ..permissions(id: builder.id.toString());
    final request = BasicRequest(route, method: 'PUT', body: jsonEncode(builder.build(includeId: false)), auditLogReason: auditLogReason);

    await client.httpHandler.executeSafe(request);
  }

  /// Delete a permission overwrite in a channel.
  Future<void> deletePermissionOverwrite(Snowflake id, Snowflake permissionId, {String? auditLogReason}) async {
    final route = HttpRoute()
      ..channels(id: id.toString())
  ...
  Future<FollowedChannel> followChannel(Snowflake id, Snowflake toFollow, {String? auditLogReason}) async {
    final route = HttpRoute()
      ...
      ..followers();
    final request = BasicRequest(route, method: 'POST', body: jsonEncode({'webhook_channel_id': id.toString()}), auditLogReason: auditLogReason);
